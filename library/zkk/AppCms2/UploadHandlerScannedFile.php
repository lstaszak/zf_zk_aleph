...
class AppCms2_UploadHandlerScannedFile
{
  ...
  function __construct($options = null, $initialize = false)
  {
    $this->options = array(
      'script_url' => $this->get_full_url() . '/',
      'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']) . '/../zkk/files_scanned/',
      'upload_url' => $this->get_full_url() . '/../zkk/files_scanned/',
      'user_dirs' => false,
      'mkdir_mode' => 0755,
      'param_name' => 'files',
  ...
  protected function trim_file_name($name, $type, $index, $content_range)
  {
    ...
    if (strpos($name, '.') === false && preg_match('/^image\/(png)/', $type, $matches)) {
      $name .= '.' . $matches[1];
    }
    return $name;
  }
