var table_order_borrower = {
  init: function(order_status_id, num_rows) {
    this.option.module = "borrower";
    this.option.baseurl = "";
  ...
  add_detete_button: function() {
    ...
    if (that.option.order_status_id === 1) {
      var $button = $("<button id=\"delete\" class=\"table_button\">" + translator.get("btn_usun") + "</button>").on("click", function() {
        ...
        dialog_confirm_del_order.init("borrower", data);
  ...
  load_data: function(reinit_pager) {
    var that = this;
    var data = {};
    var url = that.option.baseurl + "/borrower/orders/loaddataordersnew";
    var button_name = that.get_action_button_name();
    if (reinit_pager) {
      ...
      success: function(resp) {
        ...
        if (num_rows > 0) {
          ...
          $.each(rowset, function(index, value) {
            ...
            tr += "<td><p class=\"col-2\">" + value["call_id"] + "</p></td>";
            tr += "<td><p class=\"col-3\">" + value["journal_title"] + "</p></td>";
            tr += "<td><p class=\"col-4\">" + roundnumber((parseFloat(value["amount"]) / 100)) + " PLN</p></td>";
            tr += "<td><p class=\"col-5\"></p></td>";
            tr += "</tr>";
          });
          var $tr = $(tr);
          var table_width = that.option.$table.width();
          ...
          $.each(rowset, function(index, value) {
            var order_id = value["id"];
            if (value["order_status_id_is_finish"]) {
              var $td = $("#" + order_id).find("td:last");
              if (button_name !== undefined) {
                var $button = $("<p class=\"action\">" + button_name + "</p>").on("click", function() {
                  that.btn_action(that.option.order_status_id, parseInt(order_id), null);
                }).css({
              ...
              if (that.option.order_status_id === 6) {
                var $button_invoice = $("<p class=\"action\">" + "Poproś o fakturę" + "</p>").on("click", function() {
                  window.open("/borrower/payment", "_self");
                }).css({
                  "text-decoration": "underline",
                  "cursor": "pointer",
                  "font-style": "normal"
                });
                $td.append($button_invoice);
              }
              $td.append($button);
            }
          });
          $.each($tbody.find("tr").find("td:eq(3)"), function(index, value) {
          ...
          if (that.option.order_status_id === 5) {
            that.make_cart();
          }
        } else {
          $(".table_button").hide();
  ...
  get_order_settings: function(order_id) {
    ...
    $("#dialog_order_setting").empty().append(rpc_order_settings.render_order_form("borrower", order_id));
  ...
  make_cart: function() {
    var that = this;
    rpc_order_settings.init("/borrower/ordersettings");
    var cart_total_amount = rpc_order_settings.get_cart_total_amount();
    if (cart_total_amount !== null) {
      ...
      tr += "<td></td>";
      tr += "<td></td>";
      tr += "<td id=\"total_amount\"><p>Suma:<br /><span>" + roundnumber((parseInt(cart_total_amount) / 100)) + " PLN</span></p></td>";
      tr += "<td></td>";
      tr += "</tr>";
      ...
      var $button = $("<button id=\"pay\">" + translator.get("btn_zaplac") + "</button>").on("click", function() {
        window.location = "/borrower/payment/newpayment";
      }).button().css({
        "margin-left": "0px"
      });
...
var order_setting = {
  ...
  display: function() {
    this.window.attr("title", "Szczegóły zamówienias");
    this.window.dialog({
      autoOpen: false,
      width: 985,
      height: "auto",
