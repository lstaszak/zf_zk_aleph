...
(function($) {
  ...
  $.widget("ui.selectmenu", {
    ...
    _create: function() {
      ...
      this.element.bind({
        'click.selectmenu': function(event) {
          var size = self._optionLis.size();
          if (self._selectedIndex() == size - 1) {
            self._moveSelection((-1) * size);
          } else {
    ...
    _refreshPosition: function() {
      ...
      $.each(this.listWrap, function(index, value) {
        var id = $(value).find("ul").attr("id").split("-")[0];
        var button_offset = $("#" + id + "-button").offset();
        var button_height = $("#" + id + "-button").height();
        $(value).css({
